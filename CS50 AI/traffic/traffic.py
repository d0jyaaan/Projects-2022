...
def load_data(data_dir):
    ...
    # assign values to images and labels
    for i in range(0, NUM_CATEGORIES, 1):
    # for i in range(0, len(label_options), 1):
        for image in image_paths[i]:
            
            # resize
            img = cv2.imread(f"{image}")
            resized = cv2.resize(img, dim, interpolation=INTER_AREA)

            # make sure shape is 30 30 3
            if resized.shape != (30, 30, 3):
                print("SHAPE ERROR")

            # append
            images.append(resized)
            labels.append(int(label_options[i]))

    # check whether the length of images and labels are the same
